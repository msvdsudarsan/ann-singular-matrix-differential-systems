...
- Cholesky-type parameterisation `P = L*L'` guaranteeing exact symmetry and positive semi-definiteness for all network parameters (Theorem 3.3)
...
### Problem 1: Singularly Perturbed Boundary Value Problem (Table 2, Fig. 1)
...
- Finite difference (uniform) MAE = 8.12 × 10⁻⁴
- bvp4c (adaptive) MAE = 1.85 × 10⁻⁶
- PINN uniform collocation MAE = 4.77 × 10⁻¹
- Adaptive PINN MAE = (3.11 ± 2.53) × 10⁻⁶  
...
### Problem 2: Pantograph Delay Differential Equation (Table 3, Fig. 2)
...
**Key results (MATLAB-verified, 12 May 2026):**  
- RK with interpolation MAE = 1.04 × 10⁻¹
- dde23 (MATLAB, constant lag=0.5) MAE = 1.10 × 10⁻²
...
### Problem 3: Matrix Riccati Differential Equation (Table 4, Fig. 3)
...
Guarantees symmetry and positive semi-definiteness for ALL parameter values (Theorem 3.3).

**Key results (MATLAB-verified, 12 May 2026):**  
- Standalone PINN MAE = (4.95 ± 0.31) × 10⁻³  
- Hybrid PINN + ode45 MAE = (1.48 ± 0.15) × 10⁻⁹  
- Symmetry error < 10⁻¹⁵ (identically zero by algebraic construction, Theorem 3.3)  
...
| File | Purpose |
|---|---|
| `pinn_singular_perturbation.m` | Adaptive PINN for singularly perturbed BVP (Table 2) |
| `pinn_pantograph_delay.m` | PINN for pantograph DDE with proportional delay (Table 3) |
| `pinn_matrix_riccati.m` | Cholesky-PINN + hybrid refinement for Riccati (Table 4) |
...
| `compare_uniform_pinn.m` | Uniform-collocation PINN baseline (Table 2 comparison) |
| `compare_rk4_pantograph.m` | RK4 + interpolation baseline (Table 3 comparison) |
